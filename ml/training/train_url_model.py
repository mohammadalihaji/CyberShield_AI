...
from ml.config import DATASET_DIR, MODEL_DIR, ARTIFACTS_DIR, RANDOM_STATE, N_ESTIMATORS
from ml.models.url_model import URLSecurityModel
from ml.models.model_registry import ModelRegistry
from ml.features.feature_schema import URL_FEATURE_NAMES, get_schema_metadata
...
def run_training():
    ...
    print("Loading CompPhish V4 dataset for URL Model Training...")
    df = pd.read_excel(dataset_file)
    print(f"Dataset Loaded: {df.shape[0]} samples x {df.shape[1]} columns")
    ...
    # Verify domain overlap is 0
    d_train = set(df_train["_extracted_domain"])
    d_test = set(df_test["_extracted_domain"])
    overlap = len(d_train & d_test)
    print(f"Train/Test Domain Overlap: {overlap} (Leakage check: {'PASSED' if overlap == 0 else 'FAILED'})")

    # Features and labels
    X_train = df_train[URL_FEATURE_NAMES].values.astype(np.float32)
    y_train = df_train["label"].values.astype(int)

    X_test = df_test[URL_FEATURE_NAMES].values.astype(np.float32)
    y_test = df_test["label"].values.astype(int)

    print(f"Training URL Model on {len(X_train)} samples across {len(URL_FEATURE_NAMES)} URL features...")
    url_model = URLSecurityModel(n_estimators=N_ESTIMATORS, random_state=RANDOM_STATE)
    url_model.fit(X_train, y_train)

    probas = url_model.model.predict_proba(X_test)[:, 1]
    ...
    registry.save_models(
        ...
        training_metrics={
            "url_roc_auc": float(auc),
            "url_accuracy": float(acc),
            "url_precision": float(prec),
            "url_recall": float(rec),
            "url_f1": float(f1)
        },
        dataset_info={
    ...
    print("URL model artifacts saved successfully.")
